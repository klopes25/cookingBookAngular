...
export class NotificationComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges){
    if (changes.notifs.currentValue === undefined){
      return;
    }
  ...
  getNotifsState(){
    if (this.notifs.length < 1) {
      ...
      if (this.notifs.filter((notif) => notif.state === 'error').length > 0){
        return 'error showNotif';
      }
      if (this.notifs.filter((notif) => notif.state === 'warning').length > 0){
        return 'warning showNotif';
      }
      if (this.notifs.filter((notif) => notif.state === 'success').length > 0){
        return 'success showNotif';
      }
